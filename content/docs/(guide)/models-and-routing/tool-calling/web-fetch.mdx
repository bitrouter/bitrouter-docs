...
`web_fetch` is a built-in [server tool](/docs/models-and-routing/tool-calling/server-tools): BitRouter runs the fetch itself, inside the tool-calling loop, and feeds the extracted page back to the model. So **any model routed through BitRouter gains a URL reader** — hand it a link and it gets clean, normalized page content — and the extraction engine behind it is one you bring a key for.

It's the same family as Advisor, SubAgent, Fusion, and [Web search](/docs/models-and-routing/tool-calling/websearch): off by default, enabled in config, and advertised to the model only on requests that declare it.
...
`title` and `published` are optional — no engine fills them all, so the contract is additive. The loop bounds (`max_iterations`, `tool_timeout`, …) and approval policy from [server tools](/docs/models-and-routing/tool-calling/server-tools#how-the-loop-runs) apply unchanged.
...
- [Server tools](/docs/models-and-routing/tool-calling/server-tools) — the loop that runs `web_fetch`, plus Advisor, SubAgent, and Fusion.
- [Web search](/docs/models-and-routing/tool-calling/websearch) — the sibling built-in that searches instead of fetching.
