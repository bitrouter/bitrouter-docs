...
import type { ModelRow } from "@/lib/models-catalog";
...
export function ModelsTableClient({ rows }: { rows: ModelRow[] }) {
  const [q, setQ] = useState("");
  const filtered = useMemo(() => {
    const t = q.trim().toLowerCase();
    return t
      ? rows.filter((r) => r.id.toLowerCase().includes(t) || r.name.toLowerCase().includes(t))
      : rows;
  }, [q, rows]);
  ...
  return (
    <div>
      ...
      {filtered.length === 0 && (
        <p className="py-4 text-sm text-fd-muted-foreground">No models match “{q}”.</p>
      )}
    </div>
  );
}
