...
- Self-hosting is free: run the Apache-2.0 router with your own provider keys and pay providers directly, with no BitRouter platform or request fee. With BitRouter-hosted models, you pay provider token prices with 0% token markup and model routing is included; third-party payment-processing fees may apply when adding funds. With Cloud BYOK, you keep paying providers for inference and pay BitRouter once per successful logical request for hosted routing; internal retries and fallbacks are included, and failed logical requests are not billed by BitRouter. BitRouter does not package a separate enterprise suite today; teams with deployment, security, procurement, or support requirements can work directly with the founders as early design partners.
