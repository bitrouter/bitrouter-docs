...
<Callout type="info">
The examples below use the chat completions endpoint. Support for tools (MCP) and agents (A2A) is coming soon.
</Callout>
...
<Callout type="info">
Generate a JWT with `bitrouter keygen --exp 30d --scope api --name default`. See [Auth Management](/docs/guide/auth-management) for details.
</Callout>
...
<Callout type="info">
For full auth management details including budget controls, model allowlists, and token scopes, see the [Auth Management](/docs/guide/auth-management) guide.
</Callout>
