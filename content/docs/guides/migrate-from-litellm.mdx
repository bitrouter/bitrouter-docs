...
- [MCP gateway](/docs/features/mcp) — proxy MCP servers so agents discover tools across hosts.
- [ACP gateway](/docs/features/acp) — first-class support for the Agent Client Protocol used by Claude Code, Codex, OpenCode, and others.
- [Guardrails](/docs/features/guardrails) — agent firewall on the proxy hop: inspect, redact, or block inline.
- [Observability](/docs/features/observability) — built-in spend and request tracing, no external collector required.
...
| Virtual keys + budgets + admin UI | Workspace keys (cloud); env-var keys (local) | [BYOK](/docs/features/byok), [Workspaces](/docs/cloud/workspaces) |
| Guardrails / PII / content filter | Agent firewall on the proxy hop | [Guardrails](/docs/features/guardrails) |
| Callbacks (Langfuse, Datadog, etc.) | Built-in spend + request logs; export hooks planned | [Observability](/docs/features/observability) |
| MCP Gateway | MCP gateway | [MCP](/docs/features/mcp) |
| A2A Agent Gateway | ACP gateway | [ACP](/docs/features/acp) |
| Skills Gateway / `/skills` endpoint | Skills gateway with [agentskills.io](https://agentskills.io) registry | [Agent Skills](/docs/features/agentskills) |
...
<Cards>
  ...
  <Card title="Agent features" href="/docs/features/mcp" description="MCP, ACP, skills, agent firewall, x402 payment" />
  <Card title="API Reference" href="/docs/reference/api-reference" description="OpenAI- and Anthropic-compatible endpoints" />
</Cards>
