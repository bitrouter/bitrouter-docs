...
TensorZero's gateway is provider-facing — it unifies the upstreams. BitRouter adds the **agent-facing** half: an MCP gateway for tools, an [ACP gateway](/docs/agents-and-orchestration/acp-gateway) for agent identity and dispatch, a [server-tool loop](/docs/agents-and-orchestration/tool-calling/server-tools), and agentic payment so an agent can pay per request without you provisioning a key for it.
...
<Cards>
  ...
  <Card title="Agent features" href="/docs/usage/skills" description="Skills, ACP, agent firewall, x402 payment" />
