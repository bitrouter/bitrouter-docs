...
| `mcp` | MCP gateway aggregation and caching. See [MCP gateway](/docs/usage/mcp-gateway) |
| `mcp_servers` | Upstream MCP servers, keyed by id |
| `server_tools` | MCP server ids whose tools BitRouter injects and executes itself. See [Server tools](/docs/models-and-routing/tool-calling/server-tools) |
| `agents` | Upstream ACP agents. See [ACP gateway](/docs/models-and-routing/acp-gateway) |
