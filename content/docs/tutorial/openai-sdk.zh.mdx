---
title: OpenAI SDK
description: 使用官方 OpenAI SDK 集成 AiMo Network
---

使用官方 OpenAI SDK 集成 AiMo Network，通过熟悉的 OpenAI 兼容 API 无缝访问 AI 模型。

<Callout type="info">
**认证更新：** AiMo Network 使用基于钱包的 SIWx 认证。你有三种选择：
1. **AiMo CLI**（最简单）- 参见 [AI Agent 指南](/docs/overview/ai-agent-guide)
2. **AiMo TypeScript SDK**（推荐用于应用开发）- 参见 [SDK 参考](/docs/sdk-reference)
3. **手动 SIWx**（高级）- 参见下方[高级认证](#advanced-authentication)部分
</Callout>

## 推荐方式：使用 AiMo CLI

使用 AiMo Network 最简单的方式是通过 CLI：
...
AiMo Network 使用 SIWx（Sign-In-With-X）基于钱包的认证。OpenAI SDK 需要自定义请求头注入：
