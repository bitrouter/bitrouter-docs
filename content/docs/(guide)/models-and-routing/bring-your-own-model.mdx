...
<Cards>
  <Card title="Ollama" href="/docs/guides/ollama" description="Easiest local start · :11434 · no key" />
  ...
  <Card title="Model sources" href="/docs/guides/models" description="Every way tokens can reach BitRouter, side by side" />
</Cards>
...
| `supports_store` | Whether the upstream supports the `store` field. Set to `false` for strict upstreams that reject this field. |
| `supports_stream_options` | Whether the upstream supports the `stream_options` field. Set to `false` for strict upstreams that reject this field. |

## Serving your model to others

Everything above is private: your config, your endpoint, your traffic. To make a model you host **discoverable and routable by anyone on the network** — including from BitRouter Cloud — list it in the [open registry](https://github.com/bitrouter/provider-registry) instead, by opening a PR with a provider manifest. The registry README covers the manifest schema, the `models_url` BitRouter polls, and the payment modes.
