...
| *wheel* | Scroll the pane (host scrollback for main-screen apps; forwarded to apps with mouse reporting) | Inline |
...
PTY panes support **host-owned scrollback** for main-screen applications (tmux/zellij model). When scrolled, a `↑ SCROLLBACK` hint appears; typing snaps back to the live tail.

Apps that enable **mouse reporting** (like Claude Code on the alternate screen) receive real mouse events instead of host scrollback — the wheel scrolls the app's own transcript, clicks select, and drags work natively. This happens automatically when the inner app sets the appropriate terminal modes.
...
- [Subagent](/docs/models-and-routing/tool-calling/subagent) — the routing feature underneath
