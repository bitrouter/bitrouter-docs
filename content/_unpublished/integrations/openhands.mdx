...
OpenHands talks to models through LiteLLM, which has a generic OpenAI-compatible passthrough. Configure that passthrough to hit BitRouter and OpenHands routes across the whole [registry](/docs/overview/supported-models#how-model-ids-work).
...
After the leading `openai/`, use any registry id in `provider/model` form — `openai/openai/gpt-4o`, `openai/anthropic/claude-sonnet-4-6`, `openai/google/gemini-2.5-pro` — optionally with a `:cost` or `:latency` variant on the registry id. See [Models](/docs/overview/supported-models#how-model-ids-work).
