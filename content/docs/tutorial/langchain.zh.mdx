---
title: LangChain
description: 将 AiMo Network 与 LangChain 集成，构建 AI 工作流
---

将 AiMo Network 与 LangChain 集成，使用去中心化 AI 模型构建复杂的 AI 工作流、链和代理。

<Callout type="info">
**认证更新：** AiMo Network 使用基于钱包的 SIWx 认证。你有三种选择：
1. **AiMo CLI**（最简单）- 参见 [AI Agent 指南](/docs/overview/ai-agent-guide)
2. **AiMo TypeScript SDK 与 Vercel AI SDK**（推荐用于 LangChain.js）- 见下方
...
## 替代方式：使用 AiMo SDK 与 Vercel AI SDK

对于 LangChain.js 应用，你可以将 AiMo provider 与 Vercel AI SDK 配合使用：
...
将 LangChain 与 AiMo 配合使用需要自定义 SIWx 认证：
...
LangChain 允许你通过将多个操作串联在一起来构建复杂的 AI 工作流。借助 AiMo Network，你可以在这些链中使用去中心化模型。

### 代理集成

LangChain 代理可以使用 AiMo Network 模型作为其推理引擎，实现由去中心化 AI 驱动的智能决策。

### 记忆与上下文

LangChain 的记忆功能与 AiMo Network 模型无缝配合，允许你在多次交互中保持对话上下文。
