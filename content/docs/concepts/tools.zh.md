...
当你通过 `bitrouter tui` 启动 agent 运行时时，MCP 网关会自动以 `bitrouter_tools` 的形式被注入——这是一个指向守护进程聚合 `/mcp` 端点的流式 HTTP MCP 服务器。这意味着由 TUI 生成的编排器和子代理无需额外配置即可继承你配置好的工具表面。同样的注入也发生在 AgentSkills 网关（`bitrouter_skills`）上，使每个 TUI 启动的运行时都能访问工具和技能。
