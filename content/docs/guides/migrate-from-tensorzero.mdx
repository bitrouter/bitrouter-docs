...
TensorZero's gateway is provider-facing — it unifies the upstreams. BitRouter adds the **agent-facing** half: an [MCP gateway](/docs/usage/mcp-gateway) for tools, an [ACP gateway](/docs/models-and-routing/acp-gateway) for agent identity and dispatch, a [server-tool loop](/docs/models-and-routing/tool-calling/server-tools), and agentic payment so an agent can pay per request without you provisioning a key for it.
...
| — (no equivalent) | MCP / ACP / Skills agent gateways | [Tools](/docs/usage/mcp-gateway), [Agents](/docs/models-and-routing/acp-gateway) |
...
<Cards>
  ...
  <Card title="Agent features" href="/docs/usage/mcp-gateway" description="MCP, ACP, skills, agent firewall, x402 payment" />
