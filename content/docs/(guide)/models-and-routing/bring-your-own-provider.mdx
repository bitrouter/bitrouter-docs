...
Where [bring your own model](/docs/models-and-routing/bring-your-own-model) is about weights you serve yourself, this page is about *someone else's* model reached on *your* account.
...
For a custom provider registered as `id: my-provider` in the [registry](https://github.com/bitrouter/provider-registry), set `MY_PROVIDER_API_KEY`. To use a different variable name, declare the provider explicitly in `bitrouter.yaml` with `api_key: ${MY_VAR}`.
