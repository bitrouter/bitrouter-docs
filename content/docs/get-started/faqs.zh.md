...
- **Cloud**：你想要无需管理密钥且按请求计费、无需注册供应商即可使用托管供应商网络、需要团队工作区，或需要可用性 SLA。
...
简而言之，Cloud 额外提供：托管供应商网络（[托管模型](/docs/get-started/supported-models)——无需上游密钥）、带严格作用域密钥的团队[工作区](/docs/features/namespaces)、托管可观测性控制台、托管的按请求计费，以及可用性 SLA。其余能力两种模式皆有。
