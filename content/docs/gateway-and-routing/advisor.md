...
The advisor answers and its advice is returned to the calling model, which then continues the task. Unlike [Sub-agent](/docs/gateway-and-routing/subagent), the advisor doesn't take over the work — it just answers a question.
...
- [Sub-agent](/docs/gateway-and-routing/subagent) — delegate a self-contained task instead of asking a question
- [Fusion](/docs/gateway-and-routing/fusion) — deliberate across a panel of models on one prompt
- [Model variants](/docs/gateway-and-routing/model-variants) — address a stronger variant of a model as the advisor
