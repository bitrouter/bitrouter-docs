...
Full call-chain visibility: every agent, every model, every step, with cost attributed **per run** rather than per month. A real-time CLI + TUI monitors sessions, requests, latency, and spend, and structured logs feed downstream pipelines. See [Observability](/docs/features/observability).
