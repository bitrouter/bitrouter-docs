...
For declarative cost or latency optimization across providers of a single model, see [Provider Selection](/docs/models-and-routing/provider-selection). Fallback and provider selection compose: BitRouter picks the best provider for each model in your `models` array, falling through to the next model only after the chosen provider for the current one has exhausted its retry budget.
