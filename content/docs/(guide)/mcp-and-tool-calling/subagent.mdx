...
Sub-agent spawn and prompt are **non-blocking** — the tool returns immediately with a `working` acknowledgement while the worker runs in the background. The response includes a `note` instructing the model to poll for completion rather than retrying. Use `subagent_status` to check progress:

| Status | Meaning |
| --- | --- |
| `working` | Still running — poll again |
| `completed` | Finished — response carries `reply`, `stop_reason`, `diff_stat`, and `result`/`schema_ok` |

Only **one turn per session** can run at a time. A `prompt_subagent` call is refused while a turn is already in-flight for that session.
...
- [Advisor](/docs/mcp-and-tool-calling/advisor) — consult a stronger model for guidance instead of delegating a task
- [Fusion](/docs/mcp-and-tool-calling/fusion) — deliberate across a panel of models on one prompt
