...
The advisor answers and its advice is returned to the calling model, which then continues the task. Unlike [Sub-agent](/docs/models-and-routing/tool-calling/subagent), the advisor doesn't take over the work — it just answers a question.
...
- [Sub-agent](/docs/models-and-routing/tool-calling/subagent) — delegate a self-contained task instead of asking a question
