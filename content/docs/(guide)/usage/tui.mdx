...
`grok` and `agy` launch with their own subscription auth rather than being pointed at the daemon; the daemon can borrow those same sessions as providers separately. See [Claude subscription](/docs/guides/claude-subscription) and [Codex subscription](/docs/guides/codex-subscription) for that pattern.
...
- [Subagent](/docs/agents-and-orchestration/tool-calling/subagent) — the routing feature underneath
