...
Cloud stores per-request **receipts**, not OpenTelemetry span waterfalls. When you need the full span tree — the ingress span, the routing decision, and a `CLIENT` span per upstream attempt — that lives in **your own OTLP collector**. Wire it up once with the open-source [OpenTelemetry](/docs/evals-and-observability/opentelemetry) export and the Activity view links out to it.
...
<Cards>
  <Card title="OpenTelemetry" href="/docs/evals-and-observability/opentelemetry" description="Self-run OTLP export — the span model, metrics, and backend recipes." />
  <Card title="Evaluation" href="/docs/evals-and-observability/evaluation" description="The outcome signal and cost metering behind these records." />
