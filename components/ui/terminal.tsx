...
export const Terminal = ({
  ...
  const isCustomChrome = header !== undefined;

  const content = (
    <div
      ref={containerRef}
      className={cn(
        "border-border bg-background z-0 flex h-full w-full flex-col border",
        !isCustomChrome && "max-h-100 max-w-lg rounded-xl",
        className,
      )}
    >
      {isCustomChrome ? (
      ...
      <pre className={cn("flex-1 p-4", bodyClassName)}>
        <code className="grid overflow-auto">{wrappedChildren}</code>
      </pre>
      {footer}
    </div>
  );
