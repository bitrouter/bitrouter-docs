...
价格以美元 / **百万 token** 计，取自当前[注册表](https://github.com/bitrouter/bitrouter/tree/main/registry)快照（每个模型价格最低的供应商；`—` 表示暂无按量计费的供应商提供）。每个模型都由一个或多个已注册供应商提供服务——完整列表以及如何注册自己的供应商，见[支持的供应商](/docs/get-started/supported-providers)。
