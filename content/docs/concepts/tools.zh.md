...
当你通过 `bitrouter tui` 启动 agent 运行时时，两个网关都会自动注入到运行时中：

- **`bitrouter_tools`** —— MCP 网关，以指向守护进程聚合 `/mcp` 端点的流式 HTTP MCP 服务器形式暴露。所有已配置的上游 MCP 服务器的工具都会加上 `{server}__` 前缀，供运行时访问。
- **`bitrouter_skills`** —— AgentSkills 网关，通过 `mcp serve --backend skills` 以 stdio 形式暴露，在已安装技能根目录上提供 `skills_search` 和 `skills_get`。

这适用于 TUI 启动的交互式编排器（PTY）和 ACP 子代理——它们都无需额外配置即可继承相同的工具和技能表面。MCP 网关直接连接到守护进程；技能网关作为运行时的 stdio 子进程运行。
