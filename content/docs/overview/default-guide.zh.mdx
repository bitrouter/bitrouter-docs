...
<Callout type="info">
以下示例使用 Chat Completions 端点。工具（MCP）和 Agent（A2A）支持即将推出。
</Callout>
...
<Callout type="info">
使用 `bitrouter keygen --exp 30d --scope api --name default` 生成 JWT。详见[认证管理](/docs/guide/auth-management)。
</Callout>
...
<Callout type="info">
有关完整的认证管理详情，包括预算控制、模型白名单和令牌作用域，请参阅[认证管理](/docs/guide/auth-management)指南。
</Callout>
