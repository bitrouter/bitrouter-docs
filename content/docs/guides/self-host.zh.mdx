...
参见 [自托管 vs 云](/docs/get-started/self-host-vs-cloud)。
...
BitRouter 按请求并向标准后端导出可观测性数据。自托管的指标通过两个面流出：

- 一个 `GET /metrics` 端点，当路由器接入了 Prometheus 风格的指标渲染器时提供服务（可观测性插件会
  接好一个）。像对待任意抓取目标一样让 Prometheus 指向它即可。
- 由 `bitrouter-observe` 插件提供的 traces 与 metrics 的 **OTLP 导出**，发往任意 OpenTelemetry
  后端（OTLP/HTTP 或 OTLP/gRPC）。

你也可以直接从 CLI 查询花费，无需搭建仪表盘；并在请求命中上游之前追踪某个模型名是如何解析的：

```bash
bitrouter route gpt-4o     # 打印某个模型的完整回退链
```

完整的导出与归因模型见 [可观测性](/docs/features/observability)。
...
<Cards>
  ...
  <Card title="自托管 vs 云" href="/docs/get-started/self-host-vs-cloud" description="选择适合你的部署模型。" />
  <Card title="可观测性" href="/docs/features/observability" description="Prometheus + OTLP 导出与按请求归因。" />
