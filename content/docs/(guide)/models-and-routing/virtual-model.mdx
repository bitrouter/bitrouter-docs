...
| `@fast:cost` | The virtual model `fast`, with the [`:cost` variant](/docs/models-and-routing/model-variants) overriding its own `sort`. |
...
Requests for `smart` route to Anthropic first and fall back to OpenAI. This composes with [bring your own model](/docs/models-and-routing/bring-your-own-model) — list your own endpoint first and a hosted model second, and you get free local inference with a hosted safety net behind one name.
...
- `routing.only` / `routing.ignore` can only *remove* providers from the eligible set — they can never add a provider the request wasn't already allowed to reach. [Your own provider keys](/docs/models-and-routing/bring-your-own-provider) still rank ahead of platform ones.
