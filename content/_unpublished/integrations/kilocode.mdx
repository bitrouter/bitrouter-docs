...
Kilo Code (VS Code / JetBrains) ships an **OpenAI Compatible** provider that takes a base URL, an API key, and a model id. Point it at BitRouter and the agent routes across the whole [registry](/docs/models-and-routing/models) instead of a single vendor.
...
The model field takes any registry id in `provider/model` form — `openai/gpt-4o`, `anthropic/claude-sonnet-4-6`, `google/gemini-2.5-pro` — optionally with a `:cost` or `:latency` variant to bias provider selection. See [Models](/docs/models-and-routing/models).
