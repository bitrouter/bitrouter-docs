...
describe("hoistDefs", () => {
  ...
  it("renames conflicting definitions and rewrites refs within each scope", () => {
    const doc = {
      paths: {
        "/a": {
          get: {
    ...
    expect(doc.paths["/a"].get.schema.$ref).toBe("#/$defs/Foo");
    expect(doc.paths["/b"].get.schema.$ref).toBe("#/$defs/Foo__2");
  });
});
