...
Then [start BitRouter and send a request](/docs/guides/models#start-bitrouter-and-send-a-request). Use the provider-qualified id `openai-codex:<model>` to pin the request to your subscription; see [Models](/docs/overview/supported-models#how-model-ids-work) for the exact ids the Codex backend serves.
