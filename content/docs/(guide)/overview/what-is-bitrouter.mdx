...
Scope isn't only models, either. A workflow's cost includes model calls, tool definitions, and the agent loops that generate them. The [MCP gateway](/docs/usage/mcp-gateway) centralizes upstream tools; [ACP adapters](/docs/usage/acp) let the same router policy cover supported coding-agent sessions; and the [BitRouter Skill](/docs/usage/skills) gives shell-capable agents a maintained operational interface.
...
<Cards>
  ...
  <Card title="BitRouter vs OpenRouter" href="/docs/overview/comparisons/openrouter" description="An honest side-by-side with the cloud catalog." />
</Cards>
