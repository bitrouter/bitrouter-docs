...
It reports the endpoint, sampler, cardinality usage, and in-flight spans from the running daemon — and reports `stopped` plus the compile-time flag when no daemon is reachable. For a Prometheus-based stack, ingest through an OpenTelemetry Collector. Span model and per-request attribution are on [OpenTelemetry](/docs/evals-and-observability/opentelemetry).
