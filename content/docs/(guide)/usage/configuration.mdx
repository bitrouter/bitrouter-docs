...
| `mcp` | MCP gateway aggregation, caching, and upstream protocol. See [MCP gateway](/docs/agents-and-orchestration/mcp-gateway) |
| `mcp_servers` | Upstream MCP servers, keyed by id. See [MCP gateway](/docs/agents-and-orchestration/mcp-gateway) |
| `server_tools` | MCP server ids whose tools BitRouter injects and executes itself. See [Server tools](/docs/agents-and-orchestration/tool-calling/server-tools) |
| `agents` | Upstream ACP agents. See [ACP gateway](/docs/agents-and-orchestration/acp-gateway) |
...
- [Self-hosting](/docs/self-hosting) — running the binary you just configured, in production
