...
Bare `bro` runs a **network-free credential probe** — BYOK env keys, the cloud session file, and the local credential store — then launches the wizard when nothing is configured. After setup, it opens the configured Code conversation. The wizard walks three steps:
