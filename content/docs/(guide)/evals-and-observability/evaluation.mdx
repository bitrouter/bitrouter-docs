...
- **Traces** — the settlement span carries cost attributes into your OTLP backend, so cost sits next to latency and outcome on every request. See [OpenTelemetry](/docs/evals-and-observability/opentelemetry).
- **Hosted** — on BitRouter Cloud, spend, tokens, and the per-request log are hosted for you; content is never stored. See [Tracing](/docs/evals-and-observability/tracing).
...
<Cards>
  ...
  <Card title="OpenTelemetry" href="/docs/evals-and-observability/opentelemetry" description="The self-run trace and metric substrate evals build on." />
  <Card title="Tracing" href="/docs/evals-and-observability/tracing" description="The hosted request view, if you'd rather not run a collector." />
</Cards>
