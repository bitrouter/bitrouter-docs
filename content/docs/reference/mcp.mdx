...
MCP is a second way to **drive** BitRouter, parallel to the [CLI](/docs/reference/cli). Where the CLI is a command-line surface onto the local daemon, the MCP server is a **tool surface** onto the same endpoint — so any MCP client (Claude, Cursor, or your own agent) can talk to BitRouter as a set of tools instead of shelling out.
...
One is how you reach BitRouter; the other is how BitRouter reaches your tools. See [Tools](/docs/gateway-and-routing/mcp-gateway) for the gateway, and [Agent Skill](/docs/overview/quickstart) for the third way an agent drives BitRouter.
