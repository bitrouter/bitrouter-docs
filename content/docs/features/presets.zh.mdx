...
完整的 CRUD 接口——`list`、`get`、`create`、`update`、`delete`，以及 `disable`/`enable`——记录在 [管理 API](/docs/reference/api-reference/management/listRoutingPresets) 中。读取预设需要 `routing_preset:read` scope；创建或修改需要 `routing_preset:write`。
