...
The `skills` backend exposes installed skills as a standalone server so harnesses can discover and load them. When running under `bitrouter tui`, this server is automatically injected into every launched harness. See [Agent Skills](/docs/usage/skills).
...
Both are how you reach BitRouter. See [Agent Skills](/docs/usage/skills) for the other way an agent drives it.
