...
These are the same three axes described in [Provider Selection](/docs/gateway-and-routing/provider-selection), exposed as an inline shorthand. A bare model id behaves exactly like `:balanced`.
...
The model-id suffix is the supported way to choose a routing profile today. A request-body equivalent — `provider.sort` — is described in [Provider Selection](/docs/gateway-and-routing/provider-selection) but is **not yet active**; until it ships, use the suffix.
