...
| Artifact | What it is |
|----------|------------|
| `bitrouter.yaml` | An MVP routing policy to try — see [Config (YAML)](/docs/configuration/config-file) |
...
- **`local`** *(default)* — a BitRouter daemon at `http://127.0.0.1:4356`. Run [`bro start`](/docs/usage/cli) first.
...
<Cards>
  <Card title="Config (YAML)" href="/docs/configuration/config-file" description="The bitrouter.yaml reference — every block the generated policy can contain." />
  ...
  <Card title="TUI" href="/docs/usage/tui" description="Run an interactive ACP conversation with attributed session cost." />
</Cards>
