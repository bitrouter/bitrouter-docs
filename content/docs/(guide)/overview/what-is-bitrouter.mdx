...
On a strict 80-task common-valid Terminal-Bench 2.1 comparison, an initial BitRouter configuration reached **81.25% task success** versus **81.56%** for a fixed strong-model baseline and **76.50%** for same-pool random routing. Its frozen-price nominal API cost for accepted valid-path requests was **40.93% lower** than the strong-model baseline. These are research results, not provider billing or an official leaderboard submission; review the [data and protocol](https://huggingface.co/datasets/BitRouterAI/benchmarks).
...
Scope isn't only models, either. A workflow's cost includes model calls, tool definitions, and the agent loops that generate them. The [MCP gateway](/docs/usage/mcp#mcp-gateway) centralizes upstream tools; [ACP adapters](/docs/usage/acp) let the same router policy cover supported coding-agent sessions; and the [BitRouter Skill](/docs/usage/skills) gives shell-capable agents a maintained operational interface.
...
<Cards>
  ...
  <Card title="Start optimizing" href="/docs/overview/quickstart#adaptive-routing" description="Run the loop against your own workflow and publish the result." />
  <Card title="Coding agents" href="/docs/usage/coding-agents" description="Run supported harnesses interactively, headlessly, or in their native interface." />
  <Card title="Comparison" href="/docs/overview/comparison" description="Compare BitRouter with OpenRouter and LiteLLM." />
</Cards>
