...
Supply credentials through a process-manager environment file or secrets mount. Use the same released version for the binary, policy schema, and CI validator. See [Router policy](/docs/configuration/policy) for how `bitrouter.yaml` and its linked artifacts define the complete desired state.
...
`GET /health` and `GET /v1/models` are not protected by the inference authentication hook. If model and provider discovery is sensitive, restrict those paths at the reverse proxy or network layer. Use [OpenTelemetry](/docs/configuration/telemetry#self-hosted-opentelemetry) for operational telemetry.
...
Use an OpenTelemetry Collector when the destination is Prometheus-based. See [Telemetry](/docs/configuration/telemetry) for spans, usage attribution, and exporter configuration.

## Next steps

<Cards>
  <Card title="Router policy" href="/docs/configuration/policy" description="Define, validate, and apply the router's desired behavior." />
  <Card title="Telemetry" href="/docs/configuration/telemetry" description="Export telemetry and inspect routing outcomes." />
