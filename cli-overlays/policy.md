...
Routing policies are the artifact the [self-improving loop](/docs/overview/recursive-self-improvement) learns into: `init` scaffolds `policy-lock.yaml` and binds it to a preset, live traffic teaches the adequacy ledger, and `evolve --apply` folds proven downgrades back into the file. The walkthrough, table, and ledger semantics are in [Set up looping](/docs/get-started/set-up-looping).
