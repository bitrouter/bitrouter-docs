...
<Callout type="info">
**Mix local and hosted.** Declare a [virtual model](/docs/gateway-and-routing/model-fallback) whose endpoints list your local provider first and a hosted one second: requests run on local hardware for free and fail over to the hosted model on error or overload — one model name, automatic failover.
</Callout>
