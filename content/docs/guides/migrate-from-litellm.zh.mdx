...
- [MCP 网关](/docs/features/mcp) —— 代理 MCP 服务器，让 Agent 跨主机发现工具。
- [ACP 网关](/docs/features/acp) —— 一等支持 Claude Code、Codex、OpenCode 等使用的 Agent Client Protocol。
- [Guardrails](/docs/features/guardrails) —— 代理跳点上的 Agent 防火墙：原位检查、脱敏或拦截。
- [Observability](/docs/features/observability) —— 内置消费与请求追踪，无需外部收集器。
- Agent Skills 网关（即将推出）—— 按技能而非模型名称安装与路由能力。
- [Headless CLI](/docs/reference/cli) —— TUI 向导与可脚本化命令,用于配置与运维。
...
| 虚拟密钥 + 预算 + 管理 UI | Workspace 密钥（云）；环境变量密钥（本地） | [BYOK](/docs/features/byok)、[Workspaces](/docs/cloud/workspaces) |
| Guardrails / PII / 内容过滤 | 代理跳点上的 Agent 防火墙 | [Guardrails](/docs/features/guardrails) |
| Callbacks（Langfuse、Datadog 等） | 内置消费与请求日志；导出钩子规划中 | [Observability](/docs/features/observability) |
| MCP Gateway | MCP 网关 | [MCP](/docs/features/mcp) |
| A2A Agent Gateway | ACP 网关 | [ACP](/docs/features/acp) |
| Skills Gateway / `/skills` 端点 | Skills 网关 + [agentskills.io](https://agentskills.io) 注册表 | [Agent Skills](/docs/features/agentskills) |
...
<Cards>
  ...
  <Card title="Agent 功能" href="/docs/features/mcp" description="MCP、ACP、技能、Agent 防火墙、x402 支付" />
  <Card title="API 参考" href="/docs/reference/api-reference" description="OpenAI 与 Anthropic 兼容端点" />
</Cards>
