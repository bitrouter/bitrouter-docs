...
```bash
bitrouter init --yes --after exit
bitrouter init --yes --after exit -c /etc/bitrouter/bitrouter.yaml
```

Headless `bitrouter init --yes` writes a starter config with `skip_auth: true`.
Interactive `bitrouter init` asks before writing one. Edit it to
...
lifecycle commands (verified against the [CLI reference](/docs/usage/cli)):
...
See [OpenTelemetry](/docs/evals-and-tracing/opentelemetry) for the span model,
...
- **Add a content firewall** with [Guardrails](/docs/models-and-routing/guardrails) to
