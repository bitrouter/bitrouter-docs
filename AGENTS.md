...
- The docs-family navigation has **five tabs**. Four are folders marked `"root": true`: Documentation (`content/docs/(guide)/`), Self-hosting (`content/docs/self-hosting/`), Guides (`content/docs/guides/`), API Reference (`content/docs/reference/`). Their order comes from `content/docs/meta.json`. Changelog is appended explicitly by `lib/docs-tabs.tsx`, keeps its canonical `/changelog` routes and separate `content/changelog/` source, and uses the native docs layout with release versions in its sidebar.
