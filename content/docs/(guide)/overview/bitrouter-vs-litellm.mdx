...
"Minimal" here means minimal to *operate*, not minimal in ambition. An [MCP gateway](/docs/agents-and-orchestration/mcp-gateway) for tools and an [ACP gateway](/docs/agents-and-orchestration/acp-gateway) for sub-agents are both in the box — but they serve that one loop, and it's still one binary and one `bitrouter.yaml`. Anything we don't ship, you add: the [provider registry](/docs/models-and-routing/provider-selection) takes a YAML file and a PR.
